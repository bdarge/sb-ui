...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    let isAdmin = false;
    this.configService.getUser(acct.userId).subscribe({
      next: (data) => {
        if (
          !data ||
          !data.roles ||
          !data.roles.find((r) => r.id in adminRoles)
        ) {
      ...
      error: (e) => {
        this.notificationService.error(
          e ? e.message : 'Failed to get user profile. ' + e
        );
      },
    });
  }
  ...
  save() {
    if (this.userForm.valid) {
      this.configService.saveUser(this.userForm.value as User).subscribe({
        next: (user) => {
          if (user) {
