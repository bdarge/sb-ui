...
export class EditTransactionComponent implements OnInit {
  ...
  ngOnInit() {
    this.customerSearchCtrl.valueChanges
      .pipe(
        ...
        debounceTime(200),
        map((search) =>
          this.customersService.get({ search }).pipe(
            map((d) => {
              if (!d.data) {
                return [];
              }
      ...
      .subscribe({
        ...
        error: (e) => {
          ...
          this.notificationService.error(
            e && e.message ? e.message : 'Failed to load customers. ' + e
          );
        },
      });
  }
  ...
  _add(account: Account) {
    this.tService.add(account, this.form.value as Transaction).subscribe({
  ...
  _edit() {
    this.tService.update(this.form.value as Transaction).subscribe({
      next: () => this.dialogRef.close(),
      error: (err) =>
        this.notificationService.error(
          err && err.message
